...
pub mod fuin {
    ...
    pub fn issue_session(ctx:Context<IssueSesson>,vault_nonce:u64, session_nonce: u64,session_key:Pubkey,validity_in_secs: i64,daily_limit: Option<u64>,session_spend:Option<u64>)->Result<()>{
        handlers::issue_session(ctx, vault_nonce, session_nonce, session_key, validity_in_secs, daily_limit, session_spend)
    }
    ...
    pub fn update_vault(ctx:Context<UpdateVault>,_nonce:u64, new_daily_limit: Option<u64>, new_whitelist: Option<Vec<Pubkey>>)->Result<()>{
        handlers::update_vault(ctx, _nonce, new_daily_limit, new_whitelist)
    }
    ...
    pub fn session_control(
        ...
        handlers::session_control(ctx, _nv, _ns, status)
    }
